...
3. Vue 구현 과정
- Vue CLI : Vue 개발을 위한 표준 도구로 프로젝트의 구성을 도와주는 역할임. 개발에 유요한 설치 툴을 제공하며, 결론은 편하다는 것!
- `npm install -g @vue/cli` : VS code에서 해당 명령어를 통해서 설치
    - 이미 설치가 되어있는 경우에는 하단의 create만 진행하기
- `vue create vue-cli` : ~~뒤에 vue-cli는 그냥 설정한 이름. 변경 가능~~ 변경불가
- `npm run serve` : cd로 폴더 내에 들어가서 서버 실행
    - cd 폴더로 들어가면 자동으로 git init이 진행되어 있음.
